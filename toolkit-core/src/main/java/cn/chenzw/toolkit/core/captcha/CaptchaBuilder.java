...
import cn.chenzw.toolkit.core.util.ColorKit;
...
public class CaptchaBuilder {
    ...
    private void drawRandomLine(Graphics graphics) {
        graphics.setColor(ColorKit.randomColor(160, 200));
        for (int i = 0; i < 150; i++) {
    ...
    private void drawText(Graphics graphics) {
        if (!StringUtils.isEmpty(text)) {
            for (int i = 0; i < text.length(); i++) {
                graphics.setColor(ColorKit.randomColor(20, 130));
    ...
    public BufferedImage build() throws IOException {
        ...
        // 背景颜色（背景颜色不可与字体颜色冲突）
        graphics.setColor(ColorKit.randomColor(200, 255));
        // 范围
