...
public class RSAKitTests {
    ...
    public void testGetPrivateKey() throws InvalidKeySpecException, NoSuchAlgorithmException, InvalidKeyException, NoSuchPaddingException, BadPaddingException, IllegalBlockSizeException, IOException, SignatureException {
        ...
        PublicKey publicKey = RSAKit.parsePublicKey(publicKeyStr);
        PrivateKey privateKey = RSAKit.parsePrivateKey(privateKeyStr);

        /**
         * Key转Base64
         */
        String publicKeyBase64String = RSAKit.parseKeyAsBase64String(publicKey);
        Assert.assertEquals(StringUtils.remove(publicKeyStr, "\n"), publicKeyBase64String);

        String privateKeyBase64String = RSAKit.parseKeyAsBase64String(privateKey);
         ...
         * key转PEM格式
         */
        String pemString = RSAKit.parseAsPEM(publicKey);
         ...
         * 公钥加密，私钥解密
         */
        byte[] encryptContent = RSAKit.encrypt(content.getBytes(), publicKey);
        byte[] decryptContent = RSAKit.decrypt(encryptContent, privateKey);
        Assert.assertEquals(content, new String(decryptContent));

        /**
         * 私钥加密，公钥解密
         */
        byte[] encryptContent2 = RSAKit.encrypt(content.getBytes(), privateKey);
        byte[] decryptContent2 = RSAKit.decrypt(encryptContent2, publicKey);
         ...
         * 私钥签名，公钥验证
         */
        byte[] signedContent = RSAKit.sign(content.getBytes(), privateKey, RSASignatureAlgorithm.MD5withRSA);
        boolean result = RSAKit.verify(content.getBytes(), signedContent, publicKey, RSASignatureAlgorithm.MD5withRSA);
        Assert.assertTrue(result);


        KeyPair keyPair = RSAKit.createKeyPair(RSAKeySize.BIT_1024);
        String publicKeyBase64String2 = RSAKit.parseKeyAsBase64String(keyPair.getPublic());
        log.info("publicKey => {}", publicKeyBase64String2);

        String privateKeyBase64String2 = RSAKit.parseKeyAsBase64String(keyPair.getPrivate());
        log.info("privateKey => {}", privateKeyBase64String2);
        ...
        PublicKey publicKey2 = RSAKit.parsePEMAsPublicKey(pemPublicKey);
        PrivateKey privateKey2 = RSAKit.parsePEMAsPrivateKey(pemPrivateKey);
